...
*   `GET /api/quests/today`: Returns the active quest for the current `(date, phase)`. The resolver auto-expires any in-progress quest whose phase no longer matches the natural phase (e.g. weekend quest still open when intraday begins). Response includes `phase` (the *display* phase, may indicate `pending_reports`), `natural_phase` (the actual underlying phase, used for the countdown timer and active quiz), `quest` (DB row + `quiz_results` parsed), `questions` (server-side stripped — `correct` and `explanation` removed), and `current_index` (next unanswered question). In the UI, pending reports are rendered as a non-blocking banner at the top of the quest card while keeping the daily quiz fully accessible.
